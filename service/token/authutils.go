...
func FilterAccess(username string, a *token.ResourceActions) {
	...
	//@lili start check if the action need push authority
	pushFlag := 0
	for _, sc := range a.Actions {
		if sc == "push" {
			pushFlag++
		}
		if sc == "*" {
	...
	if a.Type == "repository" {
		if strings.Contains(a.Name, "/") { //Only check the permission when the requested image has a namespace, i.e. project
			...
			//@lili start check project quota
			quotaFlag := 0
			...
			if (usedNum >= quotaNum) || (usedSize >= quotaSize*quotaThreshold) {
				quotaFlag = 1
			}
			//@lili end
			//@check if the user is clair user just need pull authority
			/*
				isClairUser, err := dao.IsClairUser(username)
				if err != nil {
					log.Errorf("Error occurred in IsClairUser: %v", err)
				}
			*/
			//@lili end
			if quotaFlag == 0 || pushFlag == 2 {
			...
			/*
				if strings.Contains(permission, "R") || dao.IsProjectPublic(projectName) || isClairUser {
			*/
