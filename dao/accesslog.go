...
func GetAccessLogByRepoNameAndRepoTag(repoName string, repoTag string, username string) (int, error) {
	sql := `select log_id from access_log where repo_name = ? and repo_tag = ? and user_id = (select user_id from user where username = ?);`
	var id int
	err := GetOrmer().Raw(sql, repoName, repoTag, username).QueryRow(&id)
	return id, err
}

//GetUserImageList ...
func GetUserImageList(rn, rt string, t, userID int) ([]models.AccessLog, error) {
	o := GetOrmer()
	sql := `select user_id,project_id,repo_name,repo_tag,op_time as latest_time,operation,TIMESTAMPDIFF(DAY,op_time,now()) as overdue 
	from (select * from access_log order by op_time desc) as a 
	where repo_name = ? and repo_tag = ? and datediff(now(),op_time)>= ? and user_id = ?
	group by repo_name,repo_tag order by user_id,project_id,repo_name;`

	accessLog := []models.AccessLog{}
	_, err := o.Raw(sql, rn, rt, t, userID).QueryRows(&accessLog)

	return accessLog, err
}

func GetEmailImageList(userID, t int) ([]models.AccessLog, error) {
	o := GetOrmer()
	sql := `select user_id, project_id, repo_name, repo_tag, op_time as latest_time, operation, TIMESTAMPDIFF(DAY,op_time,now()) as overdue 
	from (select * from access_log order by op_time desc) as a 
	where user_id = ? and operation <> 'delete' and repo_tag <> 'N/A' and datediff(now(),op_time)>= ?  
	group by repo_name,repo_tag order by user_id,project_id,repo_name;`

	accessLog := []models.AccessLog{}
	_, err := o.Raw(sql, userID, t).QueryRows(&accessLog)
