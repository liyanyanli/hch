...
func AddRepPolicy(policy models.RepPolicy) (int64, error) {
	o := GetOrmer()
	sqlTpl := `insert into replication_policy (name, project_id, target_id, enabled, description, cron_str, start_time, creation_time, update_time,partial ) values (?, ?, ?, ?, ?, ?, %s, NOW(), NOW(),?)`
	var sql string
	if policy.Enabled == 1 {
	...
	r, err := p.Exec(policy.Name, policy.ProjectID, policy.TargetID, policy.Enabled, policy.Description, policy.CronStr, policy.Partial)
	if err != nil {
		return 0, err
	}
...
func FilterRepPolicies(name string, projectID int64, targetID int64) ([]*models.RepPolicy, error) {
	o := GetOrmer()
	...
	if len(name) != 0 && projectID != 0 && targetID != 0 {
		sql += `and rp.name like ? and rp.project_id = ? and rp.target_id = ?`
		args = append(args, "%"+name+"%")
...
func GetPolicyCurrentJob(policyID int64, updateTime *time.Time) (int64, error) {
	/*
		o := GetOrmer()
		sql := `select count(*) from replication_job where policy_id = ? and (status ="pending" or status ="running")`
		var total int64
		err := o.Raw(sql, policyID).QueryRow(&total)
		return total, err
	*/
	...
	if updateTime != nil {
		qs = qs.Filter("CreationTime__gte", updateTime)
	}
...
func GetPolicyRunningJob(policyID int64, updateTime *time.Time) (int64, error) {
	...
	qs1 := qs.Filter("Status", "running")
	unfinished1, err := qs1.Count()
	if err != nil {
		return 0, err
	}
	qs2 := qs.Filter("Status", "pending")
	unfinished2, err := qs2.Count()
	if err != nil {
		return 0, err
	}
	qs3 := qs.Filter("Status", "retrying")
	unfinished3, err := qs3.Count()
	if err != nil {
		return 0, err
	}

	return unfinished1 + unfinished2 + unfinished3, nil

}
func GetPolicyErrorJob(policyID int64, updateTime *time.Time) (int64, error) {
	/*
		o := GetOrmer()
		sql := `select count(*) from replication_job where policy_id = ? and (status ="pending" or status ="running")`
		var total int64
...
func GetPolicyUpdateTime(policyID int64) (*time.Time, error) {
	o := GetOrmer()
	sql := ` select update_time from replication_policy where id =?`
	var total *time.Time
	err := o.Raw(sql, policyID).QueryRow(&total)
	return total, err
...
// UpdateRepPolicyEnablement ...
func UpdatePolicyTime(id int64) error {
	o := GetOrmer()
	p := models.RepPolicy{
		ID: id,
	}
	_, err := o.Update(&p, "StartTime")
	return err
}
