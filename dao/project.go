...
func getProjects(userID int, name string, args ...int64) ([]models.Project, error) {
	...
	if userID != 0 { //get user's projects
		sql = `select distinct p.project_id, p.owner_id, p.name, p.creation_time, p.update_time, p.public, pm.role role, pq.quota_size,pq.use_size,pq.quota_num,pq.use_num
             from project p
             left join project_member pm on p.project_id = pm.project_id
             left join project_quota pq on p.project_id = pq.project_id
	 		where p.deleted = 0 and pm.user_id= ?`
		queryParam = append(queryParam, userID)
	} else { // get all projects
		sql = `select distinct p.project_id, p.owner_id, p.name, p.creation_time, p.update_time, p.public, pq.quota_size,pq.use_size,pq.quota_num,pq.use_num
             from project p left join project_quota pq on p.project_id = pq.project_id where p.deleted = 0 `
	}
...
func GetProjectID(name string) (int, error) {
	sql := `select project_id from project where name = ?`
	var num int
	err := GetOrmer().Raw(sql, name).QueryRow(&num)
	return num, err
}

func GetProjectByOwner(id int) ([]models.Project, error) {
	sql := `select * from project where owner_id = ? and deleted = 0`
	projects := []models.Project{}
	_, err := GetOrmer().Raw(sql, id).QueryRows(&projects)
	return projects, err
}

func GetProjectByUserID(id int) ([]models.Project, error) {
	sql := `select * from project where project_id in (select project_id  from project_member where user_id = ? and role <> 3) and deleted = 0`
	projects := []models.Project{}
	_, err := GetOrmer().Raw(sql, id).QueryRows(&projects)
	return projects, err
}

//@lili get the admin of current project
func GetProjectAdmin(project_id int) ([]int, error) {
	sql := `select user_id from project_member where role =1 and project_id=?`
	var u []int
	_, err := GetOrmer().Raw(sql,project_id).QueryRows(&u)
	return u, err
}
