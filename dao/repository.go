...
import (
	...
	//"docker-harbor-clair/contrib/analyze-local-images/vendor/github.com/prometheus/common/model"
)
...
func IncreasePullCount(name string) (err error) {
	...
	if num == 0 {
		...
		log.Errorf("Failed to increase repository pull count with name: %s", name)
	}
	return nil;
}
...
// DecreasePullCount ...
func DecreasePullCount(name string) error {
	o := GetOrmer()
	_, err := o.Raw(`update repository set pull_count=pull_count-1 where name=?`, name).Exec()
	return err
}

func GetByProjectId (id int) ([]models.RepoRecord,error) {
	sql := `select * from repository where project_id = ?`
	repos := []models.RepoRecord{}
