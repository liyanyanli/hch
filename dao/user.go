...
func GetEmailByUsername(userName string) (string, error) {
	sql := `select email from user where user.username = ?`
	var mail string
	err := GetOrmer().Raw(sql, userName).QueryRow(&mail)
	return mail, err
}
...
func GetAllUserID() ([]int, error) {
	...
	var u []int
	_, err := GetOrmer().Raw(sql).QueryRows(&u)
	return u, err
}
...
func GetEmailByUserId(userID int) (string, error) {
	...
	var mail string
	err := GetOrmer().Raw(sql, userID).QueryRow(&mail)
	return mail, err
}
...
func GetUserName(uid int) (string, error) {
	...
	var name string
	err := GetOrmer().Raw(sql, uid).QueryRow(&name)
	return name, err
}
