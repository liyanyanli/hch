...
func hasProjectDevRole(userID int, projectID int64) bool {
	roles, err := listRoles(userID, projectID)
	if err != nil {
		log.Errorf("error occurred in getProjectPermission: %v", err)
		return false
	...
	for _, role := range roles {
		if role.RoleID == models.PROJECTADMIN || role.RoleID == models.DEVELOPER {
			return true
		}
	}
...
// TriggerReplicationByRepository triggers the replication according to the repository  add sync
func TriggerReplicationByRepositorySync(repository string, tags []string, operation string) {
	policies, err := GetPoliciesByRepository(repository)
	if err != nil {
		log.Errorf("failed to get policies for repository %s: %v", repository, err)
		//wg.Done()
